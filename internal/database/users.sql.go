...
const downgradeUserById = `-- name: DowngradeUserById :one
UPDATE users 
...
func (q *Queries) DowngradeUserById(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, downgradeUserById, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
